...
//
// reference:
// piper//depot/google3/photos/editing/algorithms/painterly/halton_sampler.h
#ifndef INTRINSIC_EIGENMATH_HALTON_SEQUENCE_H_
#define INTRINSIC_EIGENMATH_HALTON_SEQUENCE_H_
...
//
// To get prime_number it is suggested the user employ
//
//    #include "boost/math/special_functions/prime.hpp"
//    boost::math::prime(PrimeIndex)
//
// boost::math::prime accesses the list of first 10k prime numbers in an array,
...
class HaltonSequenceEngine {
 public:
  ...
  struct Seed {
    ...
    Seed() : prime_(boost::math::prime(0)) {}

    // index of the prime number to be used as the seed
    explicit Seed(result_type value) : prime_(boost::math::prime(value)) {}
  ...
  explicit HaltonSequenceEngine() = default;
  ...
  result_type GetDim(int dim) {
    // get the prime number associated with the dimension
    uint32_t prime_number = boost::math::prime(dim);
    return HaltonSequence<result_type>(sequence_index_, prime_number);
  }
  ...
  result_type Get(int i, int dim) {
    // get the prime number associated with the dimension
    uint32_t prime_number = boost::math::prime(dim);
    return HaltonSequence<result_type>(i, prime_number);
  }
  ...
  result_type Get(int i) {
    // get the prime number associated with the dimension
    uint32_t prime_number = boost::math::prime(DimIndex);
    return HaltonSequence<result_type>(i, prime_number);
  }
...
class HaltonPointEngine {
 ...
 private:
  ...
  // float, int etc).
  HaltonSequenceEngine<typename Eigen::internal::traits<PointType>::Scalar>
      sampler_;
};
...
#endif  // INTRINSIC_EIGENMATH_HALTON_SEQUENCE_H_
