...
(defrule behavior-tree-parallel-node-predict-select-child
  "Select child for prediction if no other child has failed yet"
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  (behavior-tree-node (id ?node-id) (tree-id ?tree-id)
                      (prediction-world-id ?prediction-world-id)
                      (type PARALLEL) (predict-state SELECTED))
  ?child <- (behavior-tree-node (tree-id ?tree-id)
                                (parent-id ?node-id) (predict-state IDLE))
  ; Do not start the next child if any child failed already.
  (not (behavior-tree-node (tree-id ?tree-id) (parent-id ?node-id)
                           (predict-state FAILED)))
 =>
  (modify ?child (predict-state SELECTED)
                 (prediction-world-id (world-clone ?prediction-world-id
                   "predict_parallel" ?*TRACING-INVALID-SPAN-ID*)))
)

(defrule behavior-tree-parallel-node-predicting
  "A parallel node can be marked as predicting if all children have started to predict"
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (id ?node-id) (tree-id ?tree-id)
                               (type PARALLEL) (predict-state SELECTED))
  ; Double negation to check that all children have been selected
  (not (behavior-tree-node (tree-id ?tree-id) (parent-id ?node-id)
                           (predict-state IDLE)))
 =>
...
(defrule behavior-tree-parallel-node-predict-success
  "A parallel node succeeds prediction if all children have succeeded to predict"
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (id ?node-id) (tree-id ?tree-id)
                               (type PARALLEL)
...
(defrule behavior-tree-parallel-node-predict-failure
  "A child predict failed, let the parallel node fail prediction"
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (id ?node-id) (tree-id ?tree-id)
                               (type PARALLEL)
                               (predict-state SELECTED|PREDICTING))
  ; TODO(stoyang): Should we wait until all children have completed?
  (exists (behavior-tree-node (tree-id ?tree-id) (parent-id ?node-id)
                              (predict-state FAILED)))
 =>
