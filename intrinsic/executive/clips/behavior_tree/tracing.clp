...
(deffunction tracing-end-predict-span (?predict-action)
  "Ends a span running in ?predict-action when leaving PREDICTING state."
  (bind ?span-reference-id (fact-slot-value ?predict-action span-reference-id))
  (span-end ?span-reference-id)
)

(deffunction tracing-end-predict-span-failure (?predict-action
...
; Determines the predictions span, i.e., the span that captures all predict
; spans.
;
; This function might modify the passed ?operation fact.
;
; Args:
;   ?operation: Fact address of the operation that the predictions span is in.
;
; Returns:
;   The span-reference-id of the predictions span or an invalid span id on
;   failure.
(deffunction tracing-get-or-create-predictions-span (?operation)
  (bind ?pred-span (fact-slot-value ?operation predictions-span-reference-id))
  (if (<> ?pred-span ?*TRACING-INVALID-SPAN-ID*) then
    (return ?pred-span)
  )

  (bind ?parent-span (fact-slot-value ?operation span-reference-id))
  (if (= ?parent-span ?*TRACING-INVALID-SPAN-ID*) then
    (printout error "tracing-get-or-create-predictions-span called with
      invalid execution span." crlf)
    (return ?*TRACING-INVALID-SPAN-ID*)
  )

  (bind ?pred-span (span-start "Predictions" ?parent-span "predictions"))
  (if (= ?pred-span ?*TRACING-INVALID-SPAN-ID*) then
    (printout error "Failed to start predictions span" crlf)
    (return ?pred-span)
  )

  (modify ?operation (predictions-span-reference-id ?pred-span))
  (return ?pred-span)
)

; Starts a span for predicting ?predict-action. Typically called, when
; ?predict-action is selected.
;
; This function might modify the operation-envelope fact.
;
; Args:
;   ?predict-action: Fact address of the selected predict action.
;   ?node-name: Name of the node this action is predicting.
;   ?tree-id: Id of the tree that this predict action is in.
;
; Returns:
;   The span-reference-id of the started span or an invalid span id on failure.
(deffunction tracing-start-predict-span (?predict-action ?node-name ?tree-id)
  (bind ?operation (operation-get-by-tree ?tree-id))
  (if (or (eq ?operation nil)
          (eq (fact-slot-value ?operation span-reference-id)
              ?*TRACING-INVALID-SPAN-ID*)) then
    (return ?*TRACING-INVALID-SPAN-ID*)
  )

  (bind ?span-reference-id (fact-slot-value ?predict-action span-reference-id))
  (if (<> ?span-reference-id ?*TRACING-INVALID-SPAN-ID*) then
    (bind ?error-msg (format nil
      (str-cat "Starting a predict span for skill %s with uid %d had an active "
               "tracing span with id %d. The span will be ended by force.")
      (fact-slot-value ?predict-action skill-id)
      (fact-slot-value ?predict-action uid)
      ?span-reference-id))
    (printout error ?error-msg crlf)
    (tracing-end-predict-span-failure ?predict-action
      ?*TRACING-STATUS-ALREADY-EXISTS* ?error-msg)
  )

  (bind ?parent-span (tracing-get-or-create-predictions-span ?operation))
  (if (= ?parent-span ?*TRACING-INVALID-SPAN-ID*) then
    ; There is an execution span, thus we expect a valid parent span
    (bind ?error-msg (str-cat "Invalid parent span for predict-action with uid "
      (fact-slot-value ?predict-action uid)))
    (printout error ?error-msg crlf)
    (return ?*TRACING-INVALID-SPAN-ID*)
  )

  (bind ?action-name (fact-slot-value ?predict-action skill-id))
  (bind ?action-id (fact-slot-value ?predict-action id))
  (bind ?action-plan-id (fact-slot-value ?predict-action plan-id))
  (bind ?span-name (str-cat "Predicting (" ?action-name ")"))
  (if (neq ?node-name "") then
    (bind ?span-name (str-cat ?span-name " \"" ?node-name "\""))
  )

  (bind ?predict-span (span-start ?span-name ?parent-span "prediction"))
  (if (= ?predict-span ?*TRACING-INVALID-SPAN-ID*) then
    (bind ?error-msg (str-cat "Failed to predict span for predict-action with "
                              "uid " (fact-slot-value ?predict-action uid)))
    (printout error ?error-msg crlf)
    (return ?predict-span)
  )

  (if (neq ?action-name "") then
    (span-add-attribute ?predict-span "action_name" ?action-name)
  )
  (if (neq ?action-id 0) then
    (span-add-attribute ?predict-span "action_id" ?action-id)
  )
  (if (neq ?action-plan-id "") then
    (span-add-attribute ?predict-span "action_plan_id" ?action-plan-id)
  )
  (if (neq ?node-name "") then
    (span-add-attribute ?predict-span "bt_node_name" ?node-name)
  )
