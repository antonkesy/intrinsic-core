...
(defrule behavior-tree-sub-tree-node-predict-start
  "A subtree node directly selects the subtree's root node for prediction."
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (tree-id ?tree-id) (type SUB-TREE)
                               (sub-tree-id ?sub-tree-id)
                               (prediction-world-id ?prediction-world-id)
                               (predict-state SELECTED))
  (behavior-tree (id ?sub-tree-id) (state ACCEPTED|RUNNING)
                 (root ?sub-tree-root-id))
  ?sub-root <- (behavior-tree-node (tree-id ?sub-tree-id)
                                   (id ?sub-tree-root-id)
                                   (predict-state IDLE))
 =>
  (modify ?node (predict-state PREDICTING))
  (modify ?sub-root (predict-state SELECTED)
                    (prediction-world-id
                      (world-clone ?prediction-world-id "predict_subtree"
                        ?*TRACING-INVALID-SPAN-ID*)))
)

(defrule behavior-tree-sub-tree-node-predict-success
  "A subtree node succeeds prediction when the subtree's root succeeds
   prediction."
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (tree-id ?tree-id) (sub-tree-id ?sub-tree-id)
                               (type SUB-TREE)
                               (predict-state IDLE|PREDICTING|FAILED))
  (behavior-tree (id ?sub-tree-id) (state ACCEPTED|RUNNING)
                 (root ?sub-tree-root-id))
  (behavior-tree-node (tree-id ?sub-tree-id) (id ?sub-tree-root-id)
 ...
 =>
  ...
  ; Combine the predictions from the child into ?prediction-results
  (bind ?prediction-results
    (world-merge-predictions ?prediction-results ?prediction-proto-id))
...
(defrule behavior-tree-sub-tree-node-predict-failure
  "A subtree node fails prediction when the subtree's root fails prediction."
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (tree-id ?tree-id) (sub-tree-id ?sub-tree-id)
                               (type SUB-TREE) (predict-state PREDICTING))
  (behavior-tree (id ?sub-tree-id) (state ACCEPTED|RUNNING)
                 (root ?sub-tree-root-id))
  (behavior-tree-node (tree-id ?sub-tree-id) (id ?sub-tree-root-id)
