...
(defrule behavior-tree-retry-node-predict-start
  "A retry node directly selects its child for prediction."
 ...
 =>
  ...
  (modify ?child (predict-state SELECTED)
                 (prediction-world-id (world-clone ?prediction-world-id
                   "predict_retry" ?*TRACING-INVALID-SPAN-ID*)))
)

(defrule behavior-tree-retry-node-predict-success
  "A retry node succeeds prediction, if its child succeeds prediction."
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (tree-id ?tree-id)
                               (type RETRY)
                               (predict-state SELECTED|PREDICTING|FAILED)
                               (retry-child-id ?retry-child-id))
  (behavior-tree-node (tree-id ?tree-id) (id ?retry-child-id)
...
(defrule behavior-tree-retry-node-predict-failure
  "A child failed prediction, let the retry node fail prediction"
  (behavior-tree (id ?tree-id) (state ACCEPTED|RUNNING))
  ?node <- (behavior-tree-node (tree-id ?tree-id)
                               (type RETRY) (predict-state PREDICTING)
                               (retry-child-id ?retry-child-id))
  (behavior-tree-node (tree-id ?tree-id) (id ?retry-child-id)
