...
// TODO(b/164237300): Deprecate RemoveDuplicateVerticesFromMeshApproximate.
// Explanation: For every vertex the function checks if there is a vertex in a
// grid neighborhood that has already been deemed a survivor. If there is no
// such vertex yet, the vertex survives as well. Otherwise the vertex is snap
// rounded to the found survivor. The good thing here is that there is no loss
// in precision due to the fact that original vertices are reused. However, the
// set of survivors is solely determined by the order of the vertices. Thus,
// resulting in different results for different memory layout.
// There is also no control on the direction vertices are snap rounded and so
// one can not guarantee conservativeness.
