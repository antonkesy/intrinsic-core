...
absl::StatusOr<const aiScene*> LoadAiSceneFromFile(const std::string& filename,
  ...
  const aiScene* scene = importer.ReadFile(filename, read_options);
  if (scene == nullptr) {
    return absl::InternalError(absl::StrCat("Failed to load mesh ", filename,
                                            ". Error is\n",
...
absl::StatusOr<std::unique_ptr<aiScene>> LoadAiSceneFromFile(
    const std::string& filename, const eigenmath::Vector3d& scale) {
  const std::string extension =
      absl::AsciiStrToLower(file::Extension(filename));
