 ...
 public:
  ...
  explicit SensorComponentImpl(bool always_on, double update_rate,
      ...
      : always_on_(always_on),
        ...
        device_type_(std::string(device_type)),
  ...
  const std::string& GetDeviceName() const override { return device_name_; }
  void SetDeviceName(absl::string_view name) override {
    device_name_ = std::string(name);
  }
  const std::string& GetDeviceType() const override { return device_type_; }
  void SetDeviceType(absl::string_view type) override {
    device_type_ = std::string(type);
  }
...
SensorComponentImpl::ToProto() const {
  ...
  result.set_device_name(device_name_);
  result.set_device_type(device_type_);
