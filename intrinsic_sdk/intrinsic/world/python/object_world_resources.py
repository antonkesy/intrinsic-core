...
class WorldObject(TransformNode):
  ...
  def get_tf_name_for_root_entity(self) -> str:
    """Returns the TF name for the root entity of the object."""
    if not self._proto.root_entity_id:
      raise ValueError(
          f'Cannot determine root entity of object with name "{self.name}".'
      )

    return self.get_tf_name_for_entity(self._proto.root_entity_id)

  def get_tf_name_for_entity(self, eid: str) -> str:
    """Returns the TF name for the given entity within of the object."""

    entity_proto = self._proto.entities[eid]
    if not entity_proto:
      raise ValueError(f'Cannot find entity "{eid}" of object "{self.name}".')

    entity_name = entity_proto.name or str(eid)

    if entity_proto.ppr_component.resource_name:
      return f'{entity_proto.ppr_component.resource_name}/{self.name}/{entity_name}'
    return f'{self.name}/{entity_name}'
