...
void ThreadBody(StopToken stop_token, absl::AnyInvocable<void(StopToken)> f,
  ...
  // Initialize the tread local tracer. This is a no-op if tracing is disabled.
  if (options.GetInitializeThreadLocalTracer()) {
    const auto& tracer_init_status =
        tracing::Tracer::InitializeThreadLocalTracer(short_name);
    // TODO(b/#): Perform setup that can fail prior to entering ThreadBody and
    // use platform-default name when optional name arg is unset.
    if (!tracer_init_status.ok()) {
      LOG(ERROR) << "Failed to InitializeThreadLocalTracer for " << short_name
                 << ": " << tracer_init_status;
    }
  }

  icon::ScopedMallocGuardHook malloc_guard_hook(
      /*ignore=*/!options.IsMallocGuarded());
  if (options.IsMallocGuarded()) {
    INTRINSIC_RT_LOG(INFO) << "Thread '" << short_name
                           << "' is using malloc guard.";
    // Initialize the thread local variable. This might allocate some
    // memory (unlikely, but possible according to c++ doc). That's why this
    // needs to be done before MallocGuard is created.
    icon::ResetThreadLocalMallocViolations();
  }
  icon::MallocGuard malloc_guard(/*ignore=*/!options.IsMallocGuarded());

  if (options.IsRealtimeGuarded()) {
    // This branch is coded redundantly, because the RealTimeGuard must remain
    // in scope during the execution of f.
    intrinsic::icon::RealTimeGuard::SetCurrentThreadName(short_name);
    icon::RealTimeGuard g;
    f(stop_token);
  } else {
...
absl::Status SetSchedule(const ThreadOptions& options,
  ...
  } else if (options.IsRealtimeGuarded()) {
    INTRINSIC_RT_LOG(WARNING)
        << "No scheduling specified for realtime thread "
        << options.GetName().value_or("") << ". This may impact performance";
  ...
  if (options.GetPriority().has_value()) {
    ...
    INTRINSIC_RT_LOG(WARNING)
        << "No priority specified for realtime thread "
        << options.GetName().value_or("") << ". This may impact performance";
...
// https://linux.die.net/man/3/pthread_setaffinity_np
  ...
  if (options.GetCpuSet().empty()) {
    ...
    if (options.IsRealtimeGuarded()) {
      INTRINSIC_RT_LOG(WARNING)
          << "No CpuSet specified for realtime thread "
          << options.GetName().value_or("") << ". This may impact performance";
    }
