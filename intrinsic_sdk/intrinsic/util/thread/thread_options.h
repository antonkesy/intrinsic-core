...
class ThreadOptions {
 public:
  ...
  // Sets the thread to be realtime guarded, for the purpose of runtime
  // analysis. Setting this to true will cause INTRINSIC_ASSERT_NON_REALTIME()
  // to fail, regardless of other settings.
  ThreadOptions& SetRealtimeGuarded();

  // Configures the thread options whether to install the malloc hook in the
  // process during thread start and whether to enable the malloc guard in this
  // thread. See intrinsic/icon/utils/malloc_guard.h for more
  // information how to configure the reaction and access the violation data.
  ThreadOptions& SetMallocGuarded(bool enable = true);

  // The thread will not initialize a thread local tracer if set.
  // intrinsic/util/thread/rt_trace_internal.h
  ThreadOptions& SetSkipInitializingThreadLocalTracer();

  // Returns "true" if a thread_local tracer should be initialized.
  bool GetInitializeThreadLocalTracer() const {
    return initialize_thread_local_tracer_;
  }
  ...
  // Returns whether the thread is realtime guarded.
  bool IsRealtimeGuarded() const { return is_realtime_guarded_; }

  // Returns whether the thread is malloc guarded.
  bool IsMallocGuarded() const { return is_malloc_guarded_; }
 ...
 private:
  ...
  bool is_realtime_guarded_{false};
  // Not part of equals check.
  bool is_malloc_guarded_{false};
  // Not part of equals check.
  bool initialize_thread_local_tracer_{true};
