...
def _unpack_run_metadata(
  ...
  # TODO(b/552306959) Simplify this if possible.
...
class Executive:
  ...
  _last_execution_time_window: Optional[structured_logging.EventSourceWindow]  # pylint: disable=line-too-long 
  ...
  def __init__(
    ...
    self._last_execution_time_window = None
  ...
  # TODO(b/403641272) reenable this when the arg is removed
  ...
  def run(
    ...
    window = structured_logging.EventSourceWindow()
    ...
    window.end_time = datetime.datetime.now()
    self._last_execution_time_window = window
  ...
  @property
  def last_execution_time_window(
      self,
  ) -> Optional[structured_logging.EventSourceWindow]:
    """Returns the start and end timestamps of the last run.

    The returned time window can be used to query the logs for the last run.

    Returns:
      The start and end timestamps of the last run. Returns None if nothing
      was run before or the last run was done with run_async.
    """
    return self._last_execution_time_window
