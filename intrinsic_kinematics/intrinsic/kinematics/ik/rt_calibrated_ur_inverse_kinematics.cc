...
constexpr double kNewtonRaphsonEpsilon = 0.0001;
constexpr size_t kNewtonRaphsonMaxIterations = 20;

}  // namespace

absl::StatusOr<std::unique_ptr<RtCalibratedUrInverseKinematics>>
RtCalibratedUrInverseKinematics::Create(
    Chain chain, const UrRobotType& arm_type,
    const InverseKinematicsInterface::Options& options) {
  auto ik_solver =
      absl::WrapUnique(new RtCalibratedUrInverseKinematics(std::move(chain)));
  INTR_RETURN_IF_ERROR(ik_solver->Init(arm_type));
  return std::move(ik_solver);
}

absl::Status RtCalibratedUrInverseKinematics::Init(
    const UrRobotType& arm_type) {
...
RtCalibratedUrInverseKinematics::RealtimeComputeIK(
  ...
  auto limits_validator =
      [&limits](const intrinsic::eigenmath::VectorNd& q_test) {
  ...
  for (int i = 0; valid_solution_count < num_requested_solutions &&
    ...
    bool found_iterative_result = KinematicChainNewtonRaphsonIKSolver::IK(
        chain(), desired_base_t_tip, chain().GetTipId(),
        kNewtonRaphsonMaxIterations, kNewtonRaphsonEpsilon, limits_validator,
        candidate_solutions_span[i]);
    if (!found_iterative_result) {
      continue;
    }

    DVLOG(1) << "Calibrated IK solution: "
             << toString(candidate_solutions_span[i]);

    solutions[valid_solution_count].position = candidate_solutions_span[i];
...
RtCalibratedUrInverseKinematics::RealtimeComputeIK(
  ...
  if (!ensure_same_branch) {
    return InverseKinematicsInterfaceRT::RealtimeComputeIK(
        hint_joint_state, desired_base_t_tip, limits, solution);
  }
...
absl::StatusOr<size_t> RtCalibratedUrInverseKinematics::ComputeBranch(
