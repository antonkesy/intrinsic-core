...
#include "intrinsic/icon/skills/aio_read_input.h"
...
#include "intrinsic/icon/skills/aio_read_input.pb.h"
...
using ::intrinsic_proto::icon::AnalogSignal;
using ::intrinsic_proto::skills::AioReadInputReturnValue;

absl::Status CopyOrderedSignals(
    const google::protobuf::Map<uint32_t, AnalogSignal>& input,
    AioReadInputReturnValue& read_values) {
...
std::unique_ptr<SkillInterface> AioReadInput::CreateSkill() {
  return std::make_unique<AioReadInput>(
      std::make_unique<icon::DefaultChannelFactory>());
}

absl::StatusOr<intrinsic_proto::skills::Footprint> AioReadInput::GetFootprint(
...
AioReadInput::Execute(const ExecuteRequest& request, ExecuteContext& context) {
  INTR_ASSIGN_OR_RETURN(
      auto params,
      request.params<intrinsic_proto::skills::AioReadInputParams>(),
      _.LogError());
  ...
  auto return_value =
      std::make_unique<intrinsic_proto::skills::AioReadInputReturnValue>();
  const auto& adio_state = part_status.adio_state();

  auto aio_block = adio_state.analog_inputs().find(params.block_name());
  if (aio_block == adio_state.analog_inputs().end()) {
    return absl::NotFoundError(absl::StrCat("Block '", params.block_name(),
                                            "' not found in part status of '",
                                            part_name, "'."));
  }
  const auto& signals = aio_block->second.signals();
