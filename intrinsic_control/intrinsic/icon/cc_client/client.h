...
class Client {
 public:
  ...
  // Gets a payload for a part. Fails if the payload is not defined.
  absl::StatusOr<std::optional<RobotPayload>> GetPayload(
      absl::string_view part_name, absl::string_view payload_name) const;

  // Sets a payload for a part. Fails if a session is active or the payload is
  // invalid. After setting the payload, ICON disables and then re-enables all
  // parts.
  absl::Status SetPayload(const RobotPayload& payload,
