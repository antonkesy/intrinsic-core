...
absl::Status InitDataLogger(
  ...
  INTR_ASSIGN_OR_RETURN(
      auto channel, intrinsic::assets::dependencies::
...
absl::StatusOr<HardwareModuleRtSchedulingData> SetupRtScheduling(
  ...
  if (use_realtime_scheduling) {
    ...
    if (!disable_malloc_guard) {
      server_thread_options.SetMallocGuarded();
    } else {
      LOG(INFO) << "Disabling malloc guard by default in all custom realtime "
                   "threads.";
    }
