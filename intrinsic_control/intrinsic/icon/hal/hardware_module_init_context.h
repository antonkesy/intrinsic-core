...
class HardwareModuleInitContext {
 public:
  ...
  // Disables the malloc guard for this Hardware Module regardless of the
  // configuration file. Use this function only if the Hardware Module
  // cannot be fixed to not allocate memory in the
  // HardwareModuleInterface::ReadStatus() or
  // HardwareModuleInterface::ApplyCommand() functions.
  //
  // Note: This might impact realtime performance!
  void DisableMallocGuard() { disable_malloc_guard_ = true; }
  bool MallocGuardDisabled() const { return disable_malloc_guard_; }
 ...
 private:
  ...
  bool disable_malloc_guard_ = false;
