...
absl::StatusOr<SharedMemoryLockstep> GetSharedMemoryLockstep(
    const HardwareModuleProxy& hwm_proxy, absl::string_view memory_name) {
  INTR_ASSIGN_OR_RETURN(auto segment_name_to_file_descriptor_map,
                        hwm_proxy.GetSegmentNameToFileDescriptorMap());

  INTR_ASSIGN_OR_RETURN(auto segment,
                        ReadWriteMemorySegment<Lockstep>::Get(
                            segment_name_to_file_descriptor_map, memory_name));
  return GetSharedMemoryLockstep(segment_name_to_file_descriptor_map,
                                 memory_name);
}
