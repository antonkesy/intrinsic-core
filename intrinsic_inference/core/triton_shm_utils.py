...
def _register_shm_region(
  ...
  mpsm_handle = getattr(handle, "_mpsm_handle", None)
  actual_size = getattr(mpsm_handle, "size", None)
  if isinstance(actual_size, int) and actual_size < byte_size:
    logging.warning(
        "Shared memory region %s (%s) has stale size %d < requested %d;"
  ...
  try:
    ...
    logging.warning(
        "Initial SystemSharedMemoryRegister for %s failed; unregistering stale"
        " region and retrying...",
        name,
    )
    try:
...
def is_shm_error(error: Exception) -> bool:
  """Returns True if the exception is caused by shared memory handling."""
  shm_exc = getattr(shm, "SharedMemoryException", None)
  if isinstance(shm_exc, type) and issubclass(shm_exc, BaseException):
    if isinstance(error, shm_exc):
      return True
  ...
  if isinstance(error, grpc.RpcError):
    ...
    if details and (
        "shared memory" in details.lower() or "shared_memory" in details.lower()
    ):
      return True
  return False
...
def restore_request(
    request: service_pb2.ModelInferRequest,
    raw_input_contents: Sequence[bytes],
    had_outputs: bool,
) -> None:
  """Restores a ModelInferRequest to its pre-SHM state."""
  if not request.raw_input_contents and raw_input_contents:
    request.raw_input_contents.extend(raw_input_contents)
  if not had_outputs:
    request.ClearField("outputs")
  clear_shm_parameters(request)
...
def run_inference(
  ...
  back to the pool. If shared memory inference fails on the server (e.g. due to
  an invalid shared memory region offset/size), restores the request and falls
  back to standard gRPC ModelInfer.
  ...
  """
  saved_raw_input_contents = list(request.raw_input_contents)
  had_outputs = bool(request.outputs)
  try:
  ...
  try:
    try:
      ...
      restore_request(request, saved_raw_input_contents, had_outputs)
      raise
    ...
    try:
      ...
      if saved_raw_input_contents and is_shm_error(e):
        restore_request(request, saved_raw_input_contents, had_outputs)
        fallback_error = e
      else:
        raise
  ...
  logging.warning(
      "SHM inference failed (%s), falling back to standard gRPC ModelInfer.",
      fallback_error,
  )
