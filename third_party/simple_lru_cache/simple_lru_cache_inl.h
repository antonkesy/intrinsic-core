...
// Changes:
// - Use `double` in setTimeout() for time comparison to silence compiler
//   warning.
// - Prefix `third_party` for including `simple_lru_cache.h.
...
#include "third_party/simple_lru_cache/simple_lru_cache.h"
...
void SimpleLRUCacheBase<Key, Value, MapType, EQ>::setTimeout(double seconds,
  ...
  if (seconds < 0 || std::isinf(seconds)) {
    ...
    if (timeout_cycles >= std::numeric_limits<double>::max()) {
