...
- The hardware you want to control is physically connected to an IPC that has been configured [with k3s](/incode/ioc/setup_k3s.sh) and [real-time execution](/incode/ioc/setup_realtime.sh)
...
```sh
# Build the C++ library
bazel build //incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback:loopback_hardware_module

# Build the hardware module binary
bazel build //incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback:loopback_hardware_module_main

# Build the container image for deployment (note: building the image transitively builds the binary and library)
bazel build //incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback:loopback_hardware_module_image
```
...
```bash
bazel build //incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi:kr6_r900_2_loopback_hardware_module
```

*Expected Output:*
```text
Target //incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi:kr6_r900_2_loopback_hardware_module up-to-date:
  bazel-bin/incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi/kr6_r900_2_loopback_hardware_module.bundle.tar
...
```bash
inctl asset install bazel-bin/incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi/kr6_r900_2_loopback_hardware_module.bundle.tar \
...
[`loopback/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback/BUILD) and
[`fake/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/BUILD).
...
*   A `cc_library` to create a library containing your custom hardware module
    implementation. 
    This must implement the ([`hardware_module_interface.h`](/incode/intrinsic_control/intrinsic/icon/hal/hardware_module_interface.h)) (e.g. [`:fake_module` in `fake/BUILD#L64`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/BUILD#L64)).
    To export the module to the runtime registry ([`hardware_module_registry.h`](/incode/intrinsic_control/intrinsic/icon/hal/hardware_module_registry.h)), registration can be included directly in this library with `alwayslink = 1` (as in [`loopback/BUILD#L29`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback/BUILD#L29)), or defined in a separate registration library (e.g. [`:fake_module_register` in `fake/BUILD#L129`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/BUILD#L129)) to keep the core module decoupled for testing (see [`:fake_module_test` in `fake/BUILD#L139`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/BUILD#L139)).
...
*   A `hardware_module_binary` build rule to link a custom hardware module
    ...
    executable. See [`loopback/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback/BUILD#L78).

*   A `hardware_module_image` build rule that packages the hardware module binary 
    into a container image.
    (This rule can also take `hardware_module_lib` directly to build the binary and image together, as shown in [`fake/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/BUILD#L187)).

*   A `hardware_module_manifest` build rule that defines the `intrinsic_service`
    metadata. This creates a `ServiceManifest` ([`incode/intrinsic_apis/intrinsic/assets/services/proto/service_manifest.proto`](/incode/intrinsic_apis/intrinsic/assets/services/proto/service_manifest.proto)) 
    ...
    (See [`kuka_rsi/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi/BUILD#L276) for an example of packaging `fake_module_image.tar` with this rule.)

*   An `intrinsic_service` rule that bundles the hardware module and simulation stub image, the `default_config` proto, and the hardware module manifest. See [`kuka_rsi/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi/BUILD#L283).
...
*   An `intrinsic_hardware_device` rule that bundles the `intrinsic_service` with
    ...
    *   **Simulation**: Dynamics and simulation tags that allow the module to run
    ...
    (See [`kuka_rsi/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi/BUILD#L306)
    and
    [`abb/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/abb/BUILD#L53)
...
load("//incode/intrinsic_control/intrinsic/icon/hal/bzl:hardware_module_binary.bzl", "hardware_module_binary")
load("//incode/intrinsic_control/intrinsic/icon/hal/bzl:hardware_module_image.bzl", "hardware_module_image")
...
The [`HardwareModuleInterface `](/incode/intrinsic_control/intrinsic/icon/hal/hardware_module_interface.h) class defines non-real-time methods for lifecycle and fault handling, and real-time methods for the control loop.

This guide only provides complementary information to the detailed documentation of the hardware module interface provided in [`hardware_module_interface.h`](/incode/intrinsic_control/intrinsic/icon/hal/hardware_module_interface.h). 
...
The [`HardwareModuleInterface`](/incode/intrinsic_control/intrinsic/icon/hal/hardware_module_interface.h) specifies two kinds of methods:
...
Interfaces are advertised using the [`HardwareInterfaceRegistry`](/incode/intrinsic_control/intrinsic/icon/hal/hardware_interface_registry.h), which is obtained via the `HardwareModuleInitContext` passed to `Init()`.
...
[arm part](/incode/intrinsic_apis/intrinsic/icon/control/parts/hal/arm_part/hal_arm_part_config.proto) expects additional hardware interfaces. See
[`loopback_hardware_module.cc`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback/loopback_hardware_module.cc) for a
complete example.
...
The file [`default_hardware_interfaces.h`](/incode/intrinsic_control/intrinsic/icon/hal/default_hardware_interfaces.h) lists all interfaces currently supported by [HAL parts](incode/intrinsic_control/intrinsic/icon/control/parts/hal).
...
Finally, to export the hardware module to the runtime registry, use the [`REGISTER_HARDWARE_MODULE`](/incode/intrinsic_control/intrinsic/icon/hal/hardware_module_registry.h) macro in the outer scope (outside of any namespace). This can be done in either of two ways:

1. **In a separate registration file** (e.g. [`fake_module_register.cc`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/fake_module_register.cc)):
   ```cc
   ...
   ```
   In your `BUILD` file, define a dedicated `cc_library` target with `alwayslink = 1` for registration (see [`fake_module_register` in `fake/BUILD`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/BUILD#L129-L137)):
   ```python
   cc_library(
       ...
       deps = [
           ":my_module",
           "//incode/intrinsic_control/intrinsic/icon/hal:hardware_module_registry",
       ],
       alwayslink = 1,
   )
   ```
   ...
   * **Independent unit tests**: Unit test targets (such as `cc_test_and_malloc_test` in [`fake_module_test.cc`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/fake/fake_module_test.cc)) can depend directly on `:my_module`. Tests do not initialize the registry and do not cause duplicate registration errors.
...
2. **At the bottom of your module class's `.cc` file** (e.g. [`loopback_hardware_module.cc`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/loopback/loopback_hardware_module.cc)):
   ```cc
...
When the hardware module process starts, [`HardwareModuleRuntime`](/incode/intrinsic_control/intrinsic/icon/hal/hardware_module_runtime.h) calls `Init`.
...
* [`universal_robots/ur_module.cc`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/universal_robots/ur_module.cc) (specifically `RealtimeLoop` and `TickIconOnErrorResetClock`).
* [`kuka_rsi/kuka_rsi_communicator.cc`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi/kuka_rsi_communicator.cc) (specifically `TickClock` and `RsiCommunicationThreadJob`, configured via [`kuka_rsi/kuka_rsi_hardware_module.cc`](/incode/intrinsic_control/intrinsic/icon/hardware_modules/kuka_rsi/kuka_rsi_hardware_module.cc)).
