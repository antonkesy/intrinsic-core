...
class HealthService
 ...
 public:
  explicit HealthService(std::shared_ptr<SimPinchGripperImpl> gripper_impl)
  ...
  grpc::Status Enable(
    ...
    return ToGrpcStatus(gripper_impl_->Enable());
  }
  ...
  grpc::Status Disable(
    ...
    return ToGrpcStatus(gripper_impl_->Disable());
  }
  ...
  grpc::Status ClearFaults(
      ...
      override {
    return ToGrpcStatus(gripper_impl_->ClearFaults());
  }
  ...
  grpc::Status Enable(
    ...
    if (intrinsic_proto::services::v1::SelfState state =
      ...
      if (auto status = ToGrpcStatus(gripper_impl_->ClearFaults());
          !status.ok()) {
        return status;
      }
    }
